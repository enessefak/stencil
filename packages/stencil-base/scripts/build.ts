...
async function build(rootDir: string) {
  try {
    ...
    const distIoniconsDir = join(distDir, "stencil-base");
...
async function createSvgSymbols(
  version: string,
  distDir: string,
  srcSvgData: SvgData[]
) {
  ...
  const symbolsSvgFilePath = join(distDir, "stencil-base.symbols.svg");
...
async function createCheatsheet(
  ...
  const CheatsheetTmpFilePath = join(
    rootDir,
    "scripts",
...
async function createIconPackage(
  version: string,
  iconDir: string,
  srcSvgData: SvgData[]
) {
...
async function createDataJson(
  ...
  const distDataJsonPath = join(distDir, "stencil-base.json");
...
interface SvgData {
  ...
  /**
   * /dist/stencil-base/svg/airplane-outline.svg
   */
